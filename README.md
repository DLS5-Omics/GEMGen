...
> **Note**
> It is recommended to use a CUDA-enabled environment with a compatible NVIDIA GPU for efficient inference.
...
* Modify prompt templates in `data/` to adapt GEMGen to new biological contexts.
* Extend `generator.py` or `scorer.py` to integrate alternative models or scoring strategies.
* Adjust inference parameters (e.g., sampling strategy, batch size) in the demo scripts.
...
GEMGen is intended for **research use only**.
Predictions generated by this model should be validated experimentally before any clinical or commercial application.